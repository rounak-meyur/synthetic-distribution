...
@author: Rounak Meyur
Description: Creates the secondary distribution network in a given county. This 
is a program to continue the secondary network generation from a checkpoint.
The checkpoint may be caused due to forced stoppage by the admin while running 
the program in the cluster. This program skips the link which is taking a long
time to run. Change the script every time you run.
"""
...
from pyBuildSecNetlib import SecNet
...
#%% Get checkpointed information
secnet_obj = SecNet(homes,roads,H2Link)
L2Home = secnet_obj.link_to_home

with open(tmpPath+'sec-network/'+fiscode+'-link2home.txt','r') as f:
    linkdata = [temp.strip('\n').split('\t')[0].split(',') for temp in f.readlines()]
links = [(int(t[0]),int(t[1])) for t in linkdata]


dirname = 'sec-network/'
with open(tmpPath+dirname+fiscode+"-tsfr-data.csv",'r') as f_tsfr:
    tsfr_ct = len(f_tsfr.readlines())
with open(tmpPath+dirname+fiscode+"-time-stat.txt",'r') as f_time:
    link_ct = len(f_time.readlines())


#%% Create secondary network by skipping the current link
prefix = '51'+fiscode
suffix = datetime.datetime.now().isoformat().replace(':','-').replace('.','-')
count = 5000001+tsfr_ct

for link in links[9373:9374]:    
