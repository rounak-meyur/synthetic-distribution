# -*- coding: utf-8 -*-
"""
Created on Wed Jan 13 15:56:13 2021

Author: Rounak
Description: Quad tree implementation to partition points
"""
...
class QuadTree:
    ...
    def __init__(self, boundary, max_points=10, depth=0):
    ...
    def point_stat(self,stat):
        """Get a statistics of points within the leaf nodes of quad tree"""
        if not self.divided:
            stat[self.boundary]=len(self.points)
        else:
            stat = self.nw.point_stat(stat)
            stat = self.ne.point_stat(stat)
            stat = self.se.point_stat(stat)
            stat = self.sw.point_stat(stat)
        return stat
