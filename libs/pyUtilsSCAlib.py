# -*- coding: utf-8 -*-
"""
Created on Tue May 31 10:01:36 2022
...
def make_Zbus(Graph,sub):
    """
        ...
        (Table 8.1 sumarizes all four cases). Node 'sub' is considered as reference,
        and must exists within Graph's nodes.
        Syntax:
            Zbus, NodeList = make_Zbus(Graph,sub)
        :param Graph:
        :type Graph: networkx simple Graph
        :param sub: node ID of root substation node
        :type sub: int
    ...
    NodesUsed = []
    edgelist = nx.dfs_edges(Graph,sub)
    for edge in edgelist:
        BusA, BusB, w = edge[0], edge[1], Graph.edges[edge]['r']
        # BusA, BusB, w = edge[0], edge[1], complex(Graph.edges[edge]['r'],
        #                                           Graph.edges[edge]['x'])
        # print(BusA, BusB, w)
        if First:
