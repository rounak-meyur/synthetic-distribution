...
def random_spanning_tree(graph,root,dmax=140):
    link = {}
    in_tree = []
    in_tree.append(root)
...
T = random_spanning_tree(graph,sub)
...
create_network(T,synth_net)
plot_network(T)
plot_network(synth_net)

Hops = [np.array([nx.shortest_path_length(synth_net,n,sub) \
                              for n in list(synth_net.nodes())]),
        np.array([nx.shortest_path_length(T,n,sub) \
