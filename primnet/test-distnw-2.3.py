...
for sub in sublist[1:]:
    ...
    ax2.set_xlabel("Distance from substation (in km)",fontsize=30)
    ax2.set_ylabel("Voltage at node (in p.u.)",fontsize=30)
    ...
    leghands = [Line2D([0], [0], color='dodgerblue', marker='o',markersize=10,
                ...
                Line2D([0], [0], color='red', linewidth=0.8,
                    marker='o',markersize=5,label='LV secondary lines')]
    ax2.legend(handles=leghands,loc='lower left',ncol=1,prop={'size': 30})
