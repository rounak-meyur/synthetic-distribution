...
distpath = workpath + "/out/3-phase-net/"

sys.path.append(libpath)
from pyExtractDatalib import Get3PhaseNet
...
def plot_3phase(path,sub,savepath=None):
    # Get the network
    new_dist = Get3PhaseNet(path,sub)
    ...
    if savepath != None:
        fig.savefig("{}{}.png".format(savepath,str(sub)+'-3phase-net'),
                    bbox_inches='tight')
    return
    

#%% Get 3 phase network
sublist = [121143, 121144, 147793, 148717, 148718]

for sub in sublist:
    plot_3phase(distpath, sub, savepath=figpath)
